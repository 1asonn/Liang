  ...
  <template v-if="computedMenus.length">
    ...
    >
      <div v-if="!isMix" class="ant-pro-sider-logo">
        ...
        <img v-if="!isLeft || collapsed" src="@/assets/logo.png" alt="logo" />
        <h1 v-if="!collapsed">OverseaDjb</h1>
        <!-- </router-link> -->
      </div>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const isLeft = computed(() => layout.value === 'left');
    ...
    const computedMenus = computed(() =>
      splitMenus.value ? findMenuChildren(props.menus, route.matched[1].path) : props.menus,
    );
