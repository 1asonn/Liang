            ...
            <template #overlay>
              <a-menu>
                <a-menu-item
                  :disabled="cacheListLength === 1"
                  key="close-other"
                  @click="closeOther(item.path)"
                >
                  关闭其他
                </a-menu-item>
                <a-menu-item key="close-left" :disabled="index === 0" @click="closeLeft(item.path)">
                  关闭到左侧
                </a-menu-item>
                <a-menu-item
                  :disabled="index + 1 === cacheListLength"
                  key="close-right"
                  @click="closeRight(item.path)"
                >
                  关闭到右侧
                </a-menu-item>
                <a-menu-item @click="handleReloadPage(item.path)">刷新当前页</a-menu-item>
              </a-menu>
            </template>
          </a-dropdown>
        </template>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const cacheListLength = computed(() => {
      return store ? store.cacheList.length : 0;
    });
    const cacheList = computed(() => {
      const res = store ? store.cacheList : [];
      return res;
    });
    ...
    console.log('sideWidth', sideWidth.value);
    const { t } = useI18n();
    ...
    const handleActiveKeyChange = (key: string) => {
      const targetItem = cacheList.value.find(item => item.path === key);
      const { route } = targetItem;
...
.ant-pro-multi-tab-wrap.ant-tabs-card.ant-tabs-top :deep(.ant-tabs-nav) {
