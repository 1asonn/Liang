  ...
  <div class="main">
    <a-form id="formLogin" layout="vertical" class="user-layout-login">
      <a-tabs centered :active-key="customActiveKey" @change="handleTabClick">
        <a-tab-pane key="tab1" :tab="$t('账号密码登录')">
          <a-alert
            v-if="isLoginError"
            type="error"
            show-icon
      ...
      <a-form-item>
        <a-checkbox v-model:checked="modelRef.rememberMe">{{ $t('自动登录') }}</a-checkbox>
      ...
      <div class="user-login-other">
        ...
        <router-link class="register" to="/user/register">{{ $t('注册账户') }}</router-link>
      </div>
    </a-form>
  </div>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const rulesRef = reactive({
      ...
      account: [
        { required: true, message: t('请输入手机号码'), type: 'string' },
    ...
    const requestFailed = (err: RequestError) => {
      ...
      notification.error({
        message: t('错误'),
        description:
          ((err.response || {}).data || {}).errorMessage || t('请求出现错误，请稍后再试'),
        duration: 4,
      });
    };
    ...
    const handleSubmit = (e: Event) => {
      e.preventDefault();
      const validateNames =
        state.customActiveKey === 'tab1' ? ['account', 'password'] : ['mobile', 'captcha'];
      state.loginBtn = true;
      validate(validateNames)
        .then(values => {
          values.verifyCode = '4712';
          values.password = md5(md5(values.password) + values.verifyCode);
          userStore
            .LOGIN({
              ...values,
              clientType: 'pc',
